...
#import <RQAnimations/RQMaterializeLabel.h>

@interface MaterializeLabelViewController ()
...
@implementation MaterializeLabelViewController
...
- (void)animate {
    ...
    self.materializeLabel.attributedText = text;
    [self.materializeLabel showTextWithAnimation:YES completion:^{
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            [self.materializeLabel hideTextWithAnimation:YES completion:^{
                self.iteration++;
                [self animate];
            }];
        });
